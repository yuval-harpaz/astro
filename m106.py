...
os.chdir('/media/innereye/My Passport/Data/JWST/data/M-106')
path = glob('*.fits')
clear = [x for x in path if 'clear' in x]
notclear = [x for x in path if 'clear' not in x]
# path = path + glob('NGC-6822-MIRI/*miri*.fits')
# filt = filt_num(clear)
# clear = np.array(clear)[np.argsort(filt)]
##  'filt'
auto_plot('M-106', exp=clear, png='clear2.png', pkl=False, resize=True, method='rrgggbb', plot=False,
          crop=False, fill=False, deband=False, adj_args={'factor': 2})
