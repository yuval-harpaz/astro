...
with open('SDSSJ1723+3411.pkl', 'wb') as f:
    pickle.dump(shifted, f)
##
pow = [2, 1, 1]
auto_plot('SDSSJ1723+3411', png=True, pow=pow, pkl=True, resize=True, method='mnn')
##
path = np.asarray(list_files('/home/innereye/JWST/SDSSJ1723+3411/', '*.fits'))
filt = filt_num(path)
order = np.argsort(filt)
filt = filt[order]
path = path[order]
pow = [1, 1, 1]
auto_plot('SDSSJ1723+3411', exp=path[[0, 1, 2, 3, 4, 5, 12]], png='6layers.png', pow=pow, pkl=True, resize=True, method='mnn')

layers = np.load('SDSSJ1723+3411.pkl', allow_pickle=True)
layers = layers[15:1300,50:950,:]
for lay in range(layers.shape[2]):

# rgb = auto_plot('SDSSJ1723+3411', png=True, pkl=True, resize=False, method='rrgggbb')
