...
if os.path.isfile('bar.pkl'):
    layers = np.load('bar.pkl', allow_pickle=True)
else:
    for ii in range(len(path)):
        ...
        if ii == 0:
            ...
            orig = hdu0[1].copy()
            ref, ref_pos, ref_pix = crop_fits(orig, [8500, 2600], [1080+margins, 1920+margins])  # [4400, 6200]
            img = ref.data
...
plt.imsave('orion_bar.png',rgbt,origin='lower')
