...
# plt.figure()
for day in range(7):
    peaks = ridges(data[:, :, day], rad, tangential_smooth=10, prominence=prom, height=height, center=[150,150], smooth=False, plot=False)
...
for day in range(1,7):
    ...
    for il in range(3):
        ...
        if ll[0] > ll[-1]:
            ll = np.flipud(ll)
        if il > 0:
            length[day-1, il-1] = xx[np.where(ll < noise_thr)[0][-1]]
        plt.plot(xx, ll)
        plt.ylim(0, 1)
    plt.title('night '+str(day))
    plt.ylabel('light')
    ...
    plt.grid()
    plt.plot([0, 1000], [noise_thr, noise_thr], 'k:')
