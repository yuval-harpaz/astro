...
os.chdir('/media/innereye/My Passport/Data/JWST/data/NGC-1052-MIRI/')
path = glob('*o011*')
filt = filt_num(path)
order = np.argsort(filt)
path = np.asarray(path)[order]
pathbg = glob('*o012*')
filt = filt_num(pathbg)
order = np.argsort(filt)
pathbg = np.asarray(pathbg)[order]
path = np.concatenate([path, pathbg])
layers = reproject(path[:3])
layersbg = reproject(path[3:])

def la(fits_arr):
    fits_arr = level_adjust(fits_arr, negative='ignore', lims=[0.03, 0.98], factor=4.0, ignore0=True)
    return fits_arr

for shift in [0, 1, 2, 3]:
    rgb = np.zeros([layers.shape[0], layers.shape[1], 3])
    rgb[..., 0] = la(layers[..., 0])
    rgb[:layersbg.shape[0], shift:layersbg.shape[1]+shift, 2] = la(layersbg[..., 0])
    plt.figure()
    plt.imshow(rgb)
...
rgb = np.zeros([layers.shape[0], layers.shape[1], 3])
for ii in range(3):
    dif = layers[:layersbg.shape[0], shift:layersbg.shape[1] + shift, 2-ii].astype(float) \
        - layersbg[..., 2-ii].astype(float)
    rgb[:layersbg.shape[0], shift:layersbg.shape[1] + shift, 2-ii] = level_adjust(dif, lims=[0.25, 0.98])
    # rgb[..., 2-ii] = level_adjust(layers[..., ii] - layers[..., ii+3])
plt.figure()
plt.imshow(rgb, origin='lower')
...
rgbdb = rgb.copy()
for ii in range(3):
    rgbdb[..., ii] = deband_layer(rgbdb[..., ii].T).T
plt.figure()
plt.imshow(rgbdb, origin='lower')
##
