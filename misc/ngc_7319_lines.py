...
tableFS = get_lines(source='FSlines')
...
# path = list_files('/home/innereye/JWST/Quintet/MAST_2022-08-25T1219/JWST/', search='*s3d.fits')
    include = 'jw02732-c1001_t004_miri_'
    download_fits(object, extension=ext[1:], mrp=True, include=include, ptype='cube')
...
for ii, fn in enumerate(path1):
    ...
    # if len(peaks[ii]) > 0:
    #     plt.plot(xx[ii][peaks[ii]],yy[ii][peaks[ii]], '.k')

plt.title('Flux for NGC 7319 (AGN), extracted 1d')
plt.ylabel(hdu[1].header['TUNIT2'])
plt.xlabel('Wavelength (µm)')
txt = ['Fe', 'H₂', 'H₂', 'Ar', 'Ne', 'S', 'Ne', 'Ne', 'Ne', 'S']
ch = [0, 0, 0, 0, 1, 1, 2, 2, 2, 3]
idx = [0, 1, 4, 5, 0, 4, 1, 2, 3, 0]
for ii in range(len(txt)):
    x = peaks[ch[ii]][idx[ii]]
    y = yy[ch[ii]][x]
    plt.text(xx[ch[ii]][x],y+0.01,txt[ii],ha='center')
plt.grid()
plt.xticks(range(25))
plt.xlim(4,20)
plt.ylim(0,0.6)
# plt.show(block=False)
NeObs = xx[2][peaks[2][1:4]]
# https://irsa.ipac.caltech.edu/data/SPITZER/docs/irs/irsinstrumenthandbook/25/#_Toc312238559
Ne = np.asarray([12.814, 14.322, 15.555])
zNe = np.median(NeObs/Ne-1)
# expectedNe = Ne*(z+1)
z = 0.022  # https://en.wikipedia.org/wiki/NGC_7319, https://iopscience.iop.org/article/10.1088/0004-637X/739/2/57
...
for line in nist:
    plt.plot([line*(1+z), line*(1+z)], [0, 0.1], ':', color='purple')
plt.legend(['ch1','ch2','ch3','ch4','expected (z='+str(z)+')', 'nist'])
plt.show(block=False)


expectedFS = np.asarray(tableFS['lambda'])*(z+1)
atomsFS = np.asarray(tableFS['species'])
for ii, line in enumerate(expectedFS):
    # plt.plot([line, line], [0, 0.1], 'r:')
    plt.plot([line, line], [0, 0.3], 'g:')
    plt.text(line,0.31,atomsFS[ii])
